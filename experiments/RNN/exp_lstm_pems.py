...
class Exp_LSTM_PEMS(Exp_Basic):
    def __init__(self, args):
        super(Exp_LSTM_PEMS, self).__init__(args)
    ...
    def inference(self, model, dataloader, window_size, horizon):
        ...
        with torch.no_grad():
            for i, (inputs, target) in enumerate(dataloader):
                ...
                step = 0
                forecast_steps = np.zeros([inputs.size()[0], horizon, inputs.size()[2], inputs.size()[3]], dtype=np.float64)
                # 适配迭代预测和非迭代预测
                while step < horizon:
